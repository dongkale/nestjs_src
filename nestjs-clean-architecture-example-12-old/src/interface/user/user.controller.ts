...
export class UserController {
  private userControllerApplication: UserControllerApplication =
    new UserControllerApplication();
  ...
  async findMe(@Request() req, @Res() res: Response) {
    const response = await this.userControllerApplication.findOne(
      req.user.userId,
    );
    res.status(response.statusCode).send(response.body);
  }
  ...
  async update(@Request() req, @Res() res: Response, @Body() body: any) {
    const response = await this.userControllerApplication.update(
      req.user.userId,
      body,
    );
    res.status(response.statusCode).send(response.body);
