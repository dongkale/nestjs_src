...
export class addDefaultUserUseCases {
    ...
    async execute(): Promise<UserAggregate> {
        ...
        if (defaultUser) {
            this.logger.log('AddDefaultUser execute', 'Default user already exists');
            return defaultUser;
        }
        ...
        this.logger.log('AddDefaultUser execute', 'Default user (role: "ADMINISTRATOR") have been inserted');
        return result;
    }
}
