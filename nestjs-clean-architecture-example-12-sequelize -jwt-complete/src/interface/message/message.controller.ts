...
export class MessageController {
  private readonly messageControllerApplication: MessageControllerApplication =
    new MessageControllerApplication();
  ...
  async create(@Request() req, @Res() res: Response, @Body() body: any) {
    const response = await this.messageControllerApplication.create(
      req.user.userId,
      body,
    );
  ...
  async findAll(@Request() req, @Res() res: Response) {
    const response = await this.messageControllerApplication.findAll(
      req.user.userId,
    );
